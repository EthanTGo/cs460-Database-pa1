...
def check_in():
    if request.method == 'POST':
        ...
        to_add = "insert into checks_in (business_id, user_id, check_in_date) values (" + business_id + ", " + user_id + ", DEFAULT)"
        print(to_add)
        if day_of_week != '':
            try:
                add_query(to_add)
        ...
        return render_template('pa1_website_userCheckin.html', header='Execute something to see what happens', result='')
...
def write_review():
    if request.method == 'POST':
        ...
        stars = int(request.form['options'])
        if stars != '':
            try:
                num = hash(len(retrieve_info('select count * from reviews')))
                to_add = "insert into reviews (review_id, business_id, user_id, stars, review_data, votes_funny, votes_useful, votes_cool) values ('" + num + "','" + business_id + "','" + user_id + "'," + stars + ",'" + date + "', 0, 0, 0, '" + review_text + "')"
                add_query(to_add)
